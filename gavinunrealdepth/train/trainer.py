...
class Trainer(object):
  ...
  def _fill_experience(self, sess):
    ...
    new_state, reward, terminal, pixel_change, depths = self.environment.process(action)
    
    frame = ExperienceFrame(prev_state, reward, action, terminal, pixel_change,
                            last_action, last_reward, self.environment.last_depths)
  ...
  def _process_depthr(self, sess):
    # [Depth replay]
    # Sample 20+1 frame (+1 for last next state)
    depth_experience_frames = self.experience.sample_sequence(LOCAL_T_MAX+1)

    batch_depthr_states = []
    batch_depthr_last_action_reward = []
    batch_depthr_depths = []
    
    # t_max times loop
    for frame in depth_experience_frames[1:]:
      batch_depthr_states.append(frame.state)
      last_action_reward = frame.get_last_action_reward(self.action_size)
      batch_depthr_last_action_reward.append(last_action_reward)
      batch_depthr_depths.append(frame.depths_arr)

    return batch_depthr_states, batch_depthr_last_action_reward, batch_depthr_depths
