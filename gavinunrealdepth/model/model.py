...
class UnrealModel(object):
  ...
  def _create_depth_network(self):
    # State (Image input)
    self.depth_input = tf.placeholder("float", [None, 84, 84, 3])

    # Last action and reward
    self.depth_last_action_reward_input = tf.placeholder("float", [None, self._action_size+1])

    # VR conv layers
    depth_conv_output = self._base_conv_layers(self.depth_input, reuse=True)

    # pc lastm layers
    depth_initial_lstm_state = self.lstm_cell.zero_state(1, tf.float32)
    # (Initial state is always resetted.)
    
    depth_lstm_outputs, _ = self._base_lstm_layer(depth_conv_output,
                                               self.depth_last_action_reward_input,
                                               depth_initial_lstm_state,
                                               reuse=True)
    # depth output
    self.depth_out  = self._depth_layer(depth_lstm_outputs, reuse=True)
  ...
  def run_depth_pred(self, sess, s_t, last_action_reward):
    # For display tool if needed
    depth_out = sess.run( self.depth_out,
                         feed_dict = {self.depth_input : [s_t],
                                      self.depth_last_action_reward_input : [last_action_reward]} )
    return depth_out[0]
